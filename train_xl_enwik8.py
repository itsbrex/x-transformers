...
def train(
    ...
    train_dataset = TextSamplerDataset(data_train, train_seq_len)
    val_dataset   = TextSamplerDataset(data_val, train_seq_len)
    ...
    for i in tqdm.tqdm(range(num_batches), mininterval=10., desc='training'):
        ...
        if divisible_by(i, generate_every):
            model.eval()
            inp = random.choice(val_dataset)[:seq_len]
            prime = decode_tokens(inp.cpu().numpy())
            print(f'{prime} \n\n {"*" * 100}')

            sample = model.generate(
                start_tokens = inp[None, ...],
                seq_len = generate_length
            )

            print(decode_tokens(sample[0].cpu().numpy()))
